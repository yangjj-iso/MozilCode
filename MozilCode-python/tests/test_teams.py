...
from mozilcode.teams.backend_detect import BackendDetectionError, detect_backend
from mozilcode.teams.coordinator import (
    get_coordinator_system_prompt,
    get_coordinator_user_context,
    is_coordinator_mode,
    match_session_mode,
)
from mozilcode.agents.tool_filter import (
...
class TestBackendDetect:
    ...
    def test_iterm2_with_it2(self):
        ...
        with patch.dict(os.environ, env, clear=False):
            with patch("mozilcode.teams.backend_detect.shutil.which") as mock_which:
    ...
    def test_no_backend_raises(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("TMUX", None)
            os.environ.pop("TERM_PROGRAM", None)
            with patch("mozilcode.teams.backend_detect.shutil.which", return_value=None):
...
class TestCoordinatorMode:
    ...
    def test_enabled_with_flag_and_env(self):
        with patch.dict(os.environ, {"MOZILCODE_COORDINATOR_MODE": "1"}):
            assert is_coordinator_mode(enable_flag=True) is True

    def test_flag_without_env(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MOZILCODE_COORDINATOR_MODE", None)
    ...
    def test_match_session_mode_switch_to_coordinator(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            assert os.environ.get("MOZILCODE_COORDINATOR_MODE") == "1"
...
class TestTranscript:
    ...
    def test_save_and_load(self, tmp_dir):
        ...
        from mozilcode.teams.transcript import load_transcript, save_transcript
...
class TestAgentCoordinatorIntegration:
    def test_normal_prompt(self):
        from mozilcode.prompts import build_system_prompt, BASE_PERSONA
        prompt = build_system_prompt()
        assert BASE_PERSONA in prompt

    def test_coordinator_prompt(self):
        from mozilcode.prompts import build_system_prompt
    ...
    def test_coordinator_overrides_plan(self):
        from mozilcode.prompts import build_system_prompt, PLAN_MODE_INSTRUCTIONS
