...
from mozilcode.worktree.setup import perform_post_creation_setup
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".mozilcode" / "worktrees"
        )
        self._mozilcode_dir = Path(repo_root) / ".mozilcode"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._mozilcode_dir, session)
        return session
