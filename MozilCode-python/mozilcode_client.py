"""MozilCode Daemon 客户端 — 一个交互式终端客户端，连接本地 daemon。

用法:
    .venv\\Scripts\\python.exe mozilcode_client.py
...
class MozilCodeClient:
    ...
    async def init(self) -> None:
        ...
        if data.get("status") != "ok":
            print(c("Daemon 未运行！请先启动: python -m mozilcode.daemon", Colors.RED))
            sys.exit(1)
        print(c("✓ 已连接到 MozilCode Daemon", Colors.GREEN))
