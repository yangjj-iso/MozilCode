...
public class PlanService {
  ...
  public RedeemResult redeem(int userId, String code) {
    ...
    LocalDateTime expiresAt = LocalDateTime.now().plusDays(plan.durationDays());
  ...
  public boolean addUsage(int userId, int inputTokens, int outputTokens) {
    ...
    // Resolve active subscription id first, then update (portable for PostgreSQL).
    Optional<SubscriptionInfo> active = getActiveSubscription(userId);
    if (active.isEmpty()) {
      return false;
    }
