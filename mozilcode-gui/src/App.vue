<template>
  <div class="app">
    ...
    <div class="main">
      ...
      <div class="chat" ref="chatEl" @scroll.passive="onChatScroll">
        <div class="thread">
        <div v-if="historyLoading" class="history-loading">正在加载更早消息...</div>
          ...
          <div class="composer-tools">
            ...
            <div class="composer-right">
              ...
              <div class="composer-choice" @click.stop>
                ...
                <div v-if="modelMenuOpen" class="model-settings-menu choice-menu choice-menu-right">
                  <button class="model-setting-row" type="button" @click="modelSubmenuOpen = !modelSubmenuOpen">
                    <span>模型</span>
                    <strong>{{ composerModelLabel }}</strong>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                  </button>
                  <button v-if="composerSupportsThinking" class="model-setting-row" type="button" @click="effortSubmenuOpen = !effortSubmenuOpen">
                    <span>推理强度</span>
                    <strong>{{ composerEffortLabel }}</strong>
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>
                  </button>
                  <div v-if="modelSubmenuOpen" class="model-submenu choice-menu">
                  ...
                  <div v-if="effortSubmenuOpen" class="effort-submenu choice-menu">
                    <button type="button" class="choice-item" :class="{ active: !composerThinkingEnabled }" @click="chooseThinking(false)"><span>标准</span><small>关闭推理</small></button>
                    <button type="button" class="choice-item" :class="{ active: composerThinkingEnabled }" @click="chooseThinking(true)"><span>超高</span><small>开启推理</small></button>
                  </div>
                </div>
              </div>
      ...
      <div v-if="setTab === 'model'" class="set-pane">
        ...
        <div class="official-model-card">
          <div class="set-row-l">
            ...
            <div class="set-row-desc" v-if="accountStatus.logged_in">
              已登录 {{ accountStatus.email || '账号' }} · 云端 {{ accountStatus.base_url || '—' }} · 模型 {{ accountStatus.selected_model || '未选择' }}
            </div>
            <div class="set-row-desc" v-else>登录后直接使用云端模型目录，密钥留在服务端，无需本地配置第三方 API Key。</div>
          </div>
          <div class="set-row-r">
            <button v-if="accountStatus.logged_in" type="button" class="set-del" @click="refreshAccountModels">刷新目录</button>
            <button v-if="accountStatus.logged_in" type="button" class="set-del" @click="signOutAccount">退出</button>
            <button type="button" class="set-cancel" @click="openOfficialLogin">
              {{ accountStatus.logged_in ? '切换账号' : (showAccountLoginPanel ? '收起' : '登录') }}
            </button>
          </div>
        </div>
        <div v-if="showAccountLoginPanel" class="set-form model-form account-inline-form">
          <div class="setup-section-title">{{ accountStatus.logged_in ? '切换云端账号' : '登录云端账号' }}</div>
          <input v-model="accountForm.base_url" placeholder="Cloud URL，如 http://127.0.0.1:8000" />
          <input v-model="accountForm.email" type="email" placeholder="邮箱" autocomplete="username" />
          <input v-model="accountForm.password" type="password" placeholder="密码" autocomplete="current-password" @keyup.enter="submitAccountLogin" />
          <label class="check-row"><input type="checkbox" v-model="accountForm.register" /> 注册新账号</label>
          <div class="set-form-acts">
            <button type="button" class="set-cancel" @click="showAccountLoginPanel = false">取消</button>
            <button type="button" class="set-save" :disabled="accountBusy" @click="submitAccountLogin">
              {{ accountBusy ? '处理中…' : (accountForm.register ? '注册' : '登录') }}
            </button>
          </div>
          <div v-if="accountForm.error" class="setup-error">{{ accountForm.error }}</div>
        </div>
        <div v-if="accountStatus.logged_in" class="account-models">
          <div class="set-row-h"><span>云端模型目录</span></div>
          <div v-if="accountModels.length === 0" class="set-empty">暂无可用模型，请管理员在 /ops 启用</div>
          <div v-for="m in accountModels" :key="m.name" class="set-row model-row" :class="{ active: m.name === accountStatus.selected_model }">
            <div class="set-row-l">
              <div class="set-row-title">{{ m.display_name || m.name }} <span class="set-tag">{{ m.name === accountStatus.selected_model ? '当前' : '云端' }}</span></div>
              <div class="set-row-desc mono">{{ m.name }}{{ m.provider ? ` · ${m.provider}` : '' }}</div>
            </div>
            <div class="set-row-r">
              <button v-if="m.name !== accountStatus.selected_model" class="set-del" @click="selectAccountModel(m.name)">使用</button>
            </div>
          </div>
        </div>
        <div class="set-row-h">
          <span>已配置模型</span>
          <button class="set-add" @click="startAddModel">{{ showModelForm ? '收起表单' : '＋ 添加本地模型' }}</button>
        </div>
        ...
        <div v-for="(p, idx) in configuredModelProviders" :key="providerKey(p)" class="set-row model-row" :class="{ active: idx === 0 }">
          <div class="set-row-l">
            <div class="set-row-title">{{ p.model }} <span class="set-tag">{{ providerManagedLabel(p, idx) }}</span></div>
            <div class="set-row-desc mono">{{ p.name }} · {{ p.base_url || '—' }}</div>
            <div class="set-row-desc">{{ providerManagedDesc(p) }}</div>
          </div>
          <div class="set-row-r">
            <button v-if="idx !== 0" class="set-del" @click="activateModelProvider(p.name)">设为默认</button>
            <button v-if="!isAccountProvider(p)" class="set-del" @click="startEditModel(p)">编辑</button>
            <button v-if="!isAccountProvider(p)" class="set-del" @click="deleteModelProvider(p.name)">删除</button>
          </div>
        </div>
  ...
  <div v-if="needsConfig" class="setup">
    <div class="setup-panel">
      ...
      <h2>MozilCode</h2>
      <p>登录云端账号，或配置本地模型后开始使用。</p>
      <div class="set-form model-form setup-form">
        <div class="setup-section-title">云端账号</div>
        <input v-model="accountForm.base_url" placeholder="Cloud URL，如 http://127.0.0.1:8000" />
        <input v-model="accountForm.email" type="email" placeholder="邮箱" autocomplete="username" />
        <input v-model="accountForm.password" type="password" placeholder="密码（至少 6 位）" autocomplete="current-password" />
        <label class="check-row"><input type="checkbox" v-model="accountForm.register" /> 注册新账号</label>
        <button class="set-save setup-save" :disabled="accountBusy" @click="submitAccountLogin">{{ accountBusy ? '登录中…' : (accountForm.register ? '注册并进入' : '登录并进入') }}</button>
        <div class="setup-section-title">或配置本地 Provider</div>
        <select v-model="modelConfig.protocol" @change="applyProtocolDefaults" class="set-select">
          <option value="openai">OpenAI</option>
          <option value="anthropic">Anthropic</option>
          <option value="openai-compat">OpenAI Compatible</option>
        ...
        <button class="set-save setup-save" @click="saveModelConfig">保存本地配置并进入</button>
        <div v-if="configStatus.error || accountForm.error" class="setup-error">{{ accountForm.error || configStatus.error }}</div>
      </div>
    </div>
  </div>
  ...
  <Teleport to="body">
    <div v-if="showAccountModal" class="modal-mask" @click.self="showAccountModal = false">
      <div class="modal-panel account-modal">
        <h3>{{ accountStatus.logged_in ? '切换云端账号' : '登录 MozilCode Cloud' }}</h3>
        <p class="set-desc">JWT 仅保存在本机 daemon 会话文件；推理走 /api/gateway，上游密钥不落本地。</p>
        <div class="set-form model-form">
          ...
          <label class="check-row"><input type="checkbox" v-model="accountForm.register" /> 注册新账号</label>
          <div class="set-form-acts">
            <button type="button" class="set-cancel" @click="showAccountModal = false">取消</button>
            <button type="button" class="set-save" :disabled="accountBusy" @click="submitAccountLogin">{{ accountBusy ? '处理中…' : (accountForm.register ? '注册' : '登录') }}</button>
          </div>
          <div v-if="accountForm.error" class="setup-error">{{ accountForm.error }}</div>
        </div>
      </div>
...
// In dev mode, use relative URLs so Vite's proxy forwards /api → daemon.
// In production (Tauri build), use the absolute daemon URL directly.
const API = import.meta.env.DEV
  ? ''
  : stripTrailingSlash(import.meta.env.VITE_MOZILCODE_DAEMON_HTTP || DEFAULT_DAEMON_HTTP);
const WS_URL = import.meta.env.DEV
  ? ''
  : stripTrailingSlash(import.meta.env.VITE_MOZILCODE_DAEMON_WS || httpToWs(API));
const DAEMON_TOKEN = (import.meta.env.VITE_MOZILCODE_DAEMON_TOKEN || '').trim();
...
// Safely parse JSON from a daemon response. Returns {} when the body is not
// valid JSON (e.g. Starlette's plain-text "Not Found" on 404) so callers can
// always check r.ok / d.error without catching a throw.
...
function daemonWebSocketUrl(path) {
  // In dev mode, construct WS URL from the current page location (Vite proxy).
...
const effortSubmenuOpen = ref(false);
...
const showAccountLoginPanel = ref(false);
...
const composerSupportsThinking = computed(() => Boolean(selectedComposerProvider.value?.thinking));
const composerThinkingEnabled = computed(() => {
  const provider = selectedComposerProvider.value;
  if (!provider) return false;
  if (activeSessionId.value && sessionStatus.value.provider?.name === provider.name) return Boolean(sessionStatus.value.provider.thinking);
  return Boolean(provider.thinking);
});
const composerEffortLabel = computed(() => composerThinkingEnabled.value ? '超高' : '标准');
...
function normalizeModelProvider(p) {
  return {
    ...
    managed: p.managed || (String(p.name || '').startsWith('account:') ? 'account' : 'local'),
  };
}
...
function isAccountProvider(p) {
  return Boolean(p && (p.managed === 'account' || String(p.name || '').startsWith('account:')));
}

function localModelProviders() {
  return configuredModelProviders.value.filter((p) => !isAccountProvider(p));
}

function providerManagedLabel(p, idx) {
  if (idx === 0) return '默认';
  if (isAccountProvider(p)) return '云端';
  return modelProtocolLabel(p.protocol);
}

function providerManagedDesc(p) {
  if (isAccountProvider(p)) {
    return '经云端 gateway 推理 · JWT 会话鉴权 · 无本地 API Key';
  }
  return `${p.api_key_set ? 'API Key 已设置' : '使用环境变量或无需密钥'} · ${p.thinking ? 'thinking 开启' : 'thinking 关闭'}`;
}
...
async function saveProviderList(providers, message = '模型配置已保存') {
  // 账号托管模型不写 config.yaml，只提交本地 providers
  const localOnly = (providers || [])
    .map((p) => (p && p.name !== undefined ? modelProviderPayload(p) : p))
    .filter((p) => p && !String(p.name || '').startsWith('account:'));
  try {
    const r = await daemonFetch(`${API}/api/config`, {
      method: 'POST',
    ...
    configStatus.value = { ...configStatus.value, error: e.message || '配置保存失败' };
    toast(e.message || '配置保存失败', 'err');
    return false;
  }
}

function applyAccountStatus(d) {
  accountStatus.value = {
    logged_in: Boolean(d.logged_in),
    base_url: d.base_url || 'http://127.0.0.1:8000',
    email: d.email || '',
    role: d.role || '',
    selected_model: d.selected_model || '',
  };
  if (d.base_url) accountForm.value.base_url = d.base_url;
  if (d.email) accountForm.value.email = d.email;
}

async function loadAccountStatus() {
  try {
    const r = await daemonFetch(`${API}/api/account`);
    if (!r.ok) return;
    const d = await safeJson(r);
    applyAccountStatus(d);
    if (d.logged_in) await refreshAccountModels();
  } catch {
    // daemon 旧版本可能没有 account API
  }
}

async function refreshAccountModels() {
  try {
    const r = await daemonFetch(`${API}/api/account/models`);
    const d = await safeJson(r);
    if (!r.ok) {
      toast(d.error || '拉取云端模型失败', 'err');
      return;
    }
    accountModels.value = d.models || [];
    if (d.selected_model !== undefined) {
      accountStatus.value = { ...accountStatus.value, selected_model: d.selected_model || '' };
    }
  } catch (e) {
    toast(e.message || '拉取云端模型失败', 'err');
  }
}

async function submitAccountLogin() {
  accountForm.value.error = '';
  const email = (accountForm.value.email || '').trim();
  const password = accountForm.value.password || '';
  const base_url = (accountForm.value.base_url || '').trim();
  if (!email || !password) {
    accountForm.value.error = '请填写邮箱和密码';
    return;
  }
  accountBusy.value = true;
  try {
    const r = await daemonFetch(`${API}/api/account/session`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        email,
        ...
        register: Boolean(accountForm.value.register),
      }),
    });
    const d = await safeJson(r);
    if (!r.ok) {
      accountForm.value.error = d.error || '登录失败';
      toast(accountForm.value.error, 'err');
      return;
    }
    applyAccountStatus(d);
    accountForm.value.password = '';
    showAccountModal.value = false;
    showAccountLoginPanel.value = false;
    if (Array.isArray(d.providers)) {
      applyConfigPayload({
        configured: (d.providers || []).length > 0,
        providers: d.providers,
        permission_mode: selectedMode.value || 'default',
        config_path: configStatus.value.config_path,
        applies_to: d.applies_to,
        active_sessions_unchanged: d.active_sessions_unchanged,
      });
    } else {
      await loadConfig();
    }
    await refreshAccountModels();
    if (!accountStatus.value.selected_model && accountModels.value.length) {
      await selectAccountModel(accountModels.value[0].name);
    }
    toast(accountForm.value.register ? '注册并登录成功' : '登录成功', 'ok');
    if (!activeSessionId.value && configStatus.value.configured) {
      await createSession();
    }
  } catch (e) {
    accountForm.value.error = e.message || '登录失败';
    toast(accountForm.value.error, 'err');
  } finally {
...
async function selectAccountModel(modelName) {
  if (!modelName) return false;
  try {
    const r = await daemonFetch(`${API}/api/account/model`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    await refreshAccountModels();
    toast(`已选择云端模型 ${modelName}`, 'ok');
    return true;
  } catch (e) {
    toast(e.message || '选择模型失败', 'err');
    return false;
  }
}
...
async function signOutAccount() {
  try {
    const r = await daemonFetch(`${API}/api/account/session`, { method: 'DELETE' });
    const d = await safeJson(r);
    if (!r.ok) {
      toast(d.error || '退出失败', 'err');
      return;
    }
    applyAccountStatus(d);
    accountModels.value = [];
...
async function saveModelConfig() {
  ...
  try {
    ...
    if (await saveProviderList(nextProviders, editingProviderName.value ? '模型已保存' : '模型已添加并设为默认')) {
      const wasActiveProvider = sessionStatus.value.provider?.name === editingProviderName.value;
      // A newly configured local model must replace an account gateway model
      // in the active conversation immediately; saving config only affects
      // future sessions at the daemon level.
      if (activeSessionId.value && (!editingProviderName.value || wasActiveProvider)) {
        await switchSessionProvider(candidate.name);
      }
...
async function activateModelProvider(name) {
  ...
  // 账号模型：改 selected_model，由服务侧重排；本地模型才写 config 顺序
  if (isAccountProvider(target)) {
    const model = String(target.model || name.replace(/^account:/, ''));
    return await selectAccountModel(model);
  }
  if (idx === 0) return true;
  const nextProviders = [
    modelProviderPayload(target),
    ...localModelProviders().filter((p) => p.name !== name).map(modelProviderPayload),
  ];
  return await saveProviderList(nextProviders, '默认模型已切换');
}

async function switchSessionProvider(providerName, thinking = null) {
  if (!activeSessionId.value || !providerName) return false;
  try {
    const r = await daemonFetch(`${API}/api/session/${activeSessionId.value}/provider`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ provider_name: providerName, ...(typeof thinking === 'boolean' ? { thinking } : {}) }),
    });
    const d = await safeJson(r);
    if (!r.ok) {
      toast(d.error || '切换会话模型失败', 'err');
      return false;
    }
    applySessionStatus(d);
    return true;
  } catch (e) {
    toast(e.message || '切换会话模型失败', 'err');
    return false;
  }
}
...
async function deleteModelProvider(name) {
  ...
  if (target && isAccountProvider(target)) {
    toast('云端模型请在上方目录切换或退出账号', 'err');
    return;
  }
  const localProviders = localModelProviders();
  if (localProviders.length <= 1 && !accountStatus.value.logged_in) {
    toast('至少保留一个模型', 'err');
    return;
  }
...
async function chooseComposerModel(provider) {
  ...
  // 编辑器选择只作用于当前会话；默认模型由设置页单独维护。
  if (activeSessionId.value) {
    const ok = await switchSessionProvider(provider.name);
    if (!ok) return;
    toast(`当前会话已切换到 ${provider.model || provider.name}`, 'ok');
    return;
  }
  await activateModelProvider(provider.name);
  toast(`默认模型已设为 ${provider.model || provider.name}`, 'ok');
}

async function chooseThinking(enabled) {
  const provider = selectedComposerProvider.value;
  if (!activeSessionId.value || !provider?.name) {
    toast('请先创建对话后再设置推理强度', 'err');
    return;
  }
  if (await switchSessionProvider(provider.name, enabled)) {
    effortSubmenuOpen.value = false;
    modelMenuOpen.value = false;
    toast(enabled ? '当前会话推理强度已设为超高' : '当前会话已切换为标准推理', 'ok');
  }
}
...
function openOfficialLogin() {
  accountForm.value.error = '';
  accountForm.value.password = '';
  accountForm.value.register = false;
  if (accountStatus.value.base_url) accountForm.value.base_url = accountStatus.value.base_url;
  if (accountStatus.value.email) accountForm.value.email = accountStatus.value.email;
  // 设置页内优先展开内联表单，避免弹层被盖住时“点了没反应”
  if (showSettings.value) {
    showAccountLoginPanel.value = !showAccountLoginPanel.value;
    showAccountModal.value = false;
    return;
  }
...
async function loadSkills() {
  try {
    ...
    skills.value = (await safeJson(r)).skills || [];
  } catch { skills.value = []; }
}
...
async function loadMcp() {
  try {
    ...
    mcpServers.value = (await safeJson(r)).servers || [];
...
function connectWS(sid) {
  ...
  _wsSid = sid;
  const gen = ++_wsGen;
  // Start with the newest history page; older pages load on upward scrolling.
  messages.value = [];
  historyHasMore.value = false;
  historyBefore.value = 0;
  historyLoading.value = false;
  ...
  ws = new WebSocket(daemonWebSocketUrl(`/api/stream/${sid}?limit=240`));
  ws._gen = gen;
  ...
  ws.onmessage = (ev) => {
    ...
    catch { return; } // ignore malformed messages
    ...
    if (_replaying && data.type !== 'ReplayDone') renderHistoryEvent(data);
...
function rememberEvent(data) {
  ...
  if (!id) return true; // Compatibility with pre-event-id daemon histories.
  if (_seenEventIds.has(id)) return false;
  _seenEventIds.add(id);
  return true;
}

// History must only build transcript data. It must never change live task
// state, reopen prompts, overwrite token status, or emit notifications.
function renderHistoryEvent(data) {
  if (!rememberEvent(data)) return;
  const t = data.type;
  const d = data.data || {};
  if (t === 'UserMessage') {
    const optimistic = findOptimisticUser(d.content || '');
    if (optimistic) optimistic._optimistic = false;
    else messages.value.push({ role: 'user', content: d.content || '', tools: [], _id: data.event_id || Date.now() + Math.random() });
    ...
    addToolPart(ensureAssistant(), { id: d.tool_id, name: d.tool_name, args: d.arguments, status: 'running', expanded: false });
    ...
    finishCompactPart(d);
  } else if (t === 'TurnComplete' || t === 'LoopComplete' || t === 'TaskCancelled' || t === 'ErrorEvent') {
    finalizeAssistant();
  }
}
...
function handleEvent(data) {
  ...
  if (t === 'UserMessage') {
    ...
    _replaying = false;
    historyBefore.value = Number.isInteger(d.before) ? d.before : 0;
    historyHasMore.value = Boolean(d.has_more);
    ...
    toast(`Hook ${d.hook_id || ''}: ${d.output || (d.success ? '完成' : '失败')}`, d.success ? 'ok' : 'err');
  }
}
...
async function advanceAsk() {
  if (ask.value.currentQ < ask.value.questions.length - 1) {
    ...
    try {
      ...
      toast('回答发送失败: ' + e.message, 'err');
    }
  }
}

function onChatScroll() {
  if (chatEl.value?.scrollTop <= 72) loadOlderHistory();
}

async function loadOlderHistory() {
  if (!activeSessionId.value || !historyHasMore.value || historyLoading.value) return;
  ...
  try {
    const r = await daemonFetch(`${API}/api/session/${activeSessionId.value}/history?before=${historyBefore.value}&limit=240`);
    const d = await safeJson(r);
    if (!r.ok) throw new Error(d.error || '加载历史失败');
    const currentMessages = messages.value;
    const previousReplayState = _replaying;
    _replaying = true;
    _suppressAutoScroll = true;
    messages.value = [];
    for (const event of d.events || []) renderHistoryEvent(event);
    const olderMessages = messages.value;
    messages.value = [...olderMessages, ...currentMessages];
    _replaying = previousReplayState;
    historyBefore.value = Number.isInteger(d.before) ? d.before : 0;
    historyHasMore.value = Boolean(d.has_more);
    await nextTick();
    chat.scrollTop = previousTop + chat.scrollHeight - previousHeight;
  } catch (e) {
    toast(e.message || '加载更早消息失败', 'err');
  } finally {
...
.setup-section-title { font-size: 12px; font-weight: 600; color: var(--muted); margin: 8px 0 2px; }
.account-models { margin: 0 0 14px; display: flex; flex-direction: column; gap: 6px; }
.modal-mask {
  /* Teleport 到 body 后仍保留高 z-index，覆盖 settings/setup */
...
.modal-panel {
  ...
  border-radius: 12px;
  padding: 20px;
  box-shadow: 0 18px 60px rgba(32, 36, 33, 0.18);
}
.modal-panel h3 { margin: 0 0 8px; font-size: 18px; }
.account-modal .set-form { margin-top: 10px; }
...
.model-setting-row:hover:not(.static) {
  background: rgba(185, 137, 50, 0.16);
  border-color: rgba(185, 137, 50, 0.34);
}
.model-setting-row span { font-weight: 650; }
...
.model-setting-row svg { width: 16px; height: 16px; color: var(--dim); }
.model-setting-row.static { grid-template-columns: auto minmax(0, 1fr); cursor: default; }
.model-submenu {
  position: absolute;
  right: calc(100% + 8px);
  bottom: 0;
