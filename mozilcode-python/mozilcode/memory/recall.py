...
实现了基于 LLM 的语义召回机制：在会话开始时，用最近一条用户消息
作为查询，扫描记忆目录下的 .md 文件，让 LLM 选择器选出最多 5 个
相关记忆文件，读取其内容作为 system-reminder 注入对话。

核心流程：
1. scan_memory_files(): 递归扫描记忆目录，读取 frontmatter，返回文件头列表
2. format_memory_manifest(): 格式化清单供 LLM 选择器阅读
3. find_relevant_memories(): 语义召回主函数（扫描 → 过滤 → LLM 选择 → 返回路径）
4. render_reminder(): 读取选中记忆文件内容，渲染为 system-reminder 文本

召回是 best-effort 的：选择器失败时静默返回空列表，不阻断主对话。
"""
