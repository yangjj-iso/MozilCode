...
定义了 4 种 action 类型的执行函数：
- execute_command(): 执行 shell 命令（如 lint / 格式化）
- execute_prompt(): 注入文本给 LLM（如提醒 / 修改行为）
- execute_http(): 发 HTTP 请求（如通知外部系统 / Webhook）
- execute_agent(): 派生子 Agent 处理（如自动代码审查）

execute_action() 是统一入口，根据 action.type 分发到对应执行器。
"""
