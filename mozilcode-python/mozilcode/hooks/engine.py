...
定义了 HookEngine 类，负责在 Agent 生命周期事件触发时匹配并执行 Hook。
核心能力：
- find_matching_hooks(): 根据事件名 + 条件匹配 Hook
- run_hooks(): 同步执行匹配的 Hook（阻塞 Agent 循环）
- run_pre_tool_hooks(): 执行 pre_tool_use Hook，可以 reject 拦截工具执行
- _schedule_background_hook(): 异步执行 Hook（不阻塞 Agent 循环）

还管理 prompt action 产生的注入消息和 Hook 执行结果通知。
"""
