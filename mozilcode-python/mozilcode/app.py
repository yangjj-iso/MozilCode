...
组装聊天界面、消费 Agent 事件流，处理权限/提问/计划等内联交互，
并把用户输入与会话状态接到 Agent 运行时。
"""
...
from mozilcode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from mozilcode.commands.completion import CompletionPopup
from mozilcode.commands.handlers import register_all_commands
from mozilcode.config import MCPServerConfig, ProviderConfig
from mozilcode.hooks import HookContext, HookEngine, load_hooks
from mozilcode.conversation import ConversationManager, Message
from mozilcode.mcp import MCPManager
from mozilcode.memory import (
    MemoryManager,
    Session,
    SessionManager,
    build_memory_hub,
...
from mozilcode.agents.notification import inject_task_notifications
from mozilcode.commands.handlers.tasks import create_tasks_command
from mozilcode.skills.executor import SkillExecutor
from mozilcode.skills.loader import SkillLoader
from mozilcode.commands.handlers.skill_register import register_skill_commands
...
from mozilcode.tools.load_skill import LoadSkill
from mozilcode.worktree.cleanup import start_stale_cleanup_task
from mozilcode.worktree.manager import WorktreeManager
from mozilcode.commands.handlers.worktree import create_worktree_command
from mozilcode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".mozilcode", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".mozilcode" / "history"
...
class MozilCodeApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_MOZILCODE_THEME)
        self.theme = "mozilcode"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        from mozilcode.filehistory import FileHistory
        ...
        # --- Worktree 系统初始化 ---
        from mozilcode.config import WorktreeConfig
        ...
        from mozilcode.tools.enter_worktree import EnterWorktreeTool
        from mozilcode.tools.exit_worktree import ExitWorktreeTool
        ...
        from mozilcode.commands.handlers.trace import create_trace_command
        ...
        from mozilcode.tools.synthetic_output import SyntheticOutputTool
        ...
        try:
            async for event in self.agent.run(self.conversation):
                if isinstance(event, ThinkingText):
                    ...
                    await self._handle_askuser_event(event)
    ...
    async def _show_plan_approval(self) -> None:
        from mozilcode.plan_dialog import InlinePlanWidget

        chat = self.query_one("#chat-area", VerticalScroll)
        widget = InlinePlanWidget()
        await self._mount_live(widget)
        self.call_after_refresh(chat.scroll_end, animate=False)
        try:
    ...
    def on_inline_plan_widget_responded(
        ...
        from mozilcode.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser_event(self, event: AskUserRequest) -> None:
        """Handle AskUserRequest from the Agent event stream."""
        from mozilcode.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from mozilcode.permission_dialog import InlinePermissionWidget
    ...
    def _live_anchor(self) -> Static | None:
        """流式期间用作插入锚点的实时 spinner。

        spinner 和 teammate_tree 被挂在聊天区末尾作为"实时状态区"。
        所有在流式期间追加到聊天区的正文/工具卡片/弹窗都必须插到这个
        锚点之前，否则会出现在 spinner 下方，导致后续轮次的输出与
        "思考中"动画交错错乱。spinner 不存在（未在流式）时返回 None，
        此时按普通末尾追加即可。
        """
        spinner = self._spinner_label
        if spinner is not None and spinner.parent is not None:
            return spinner
        return None

    async def _mount_live(self, widget: Any) -> None:
        """把 widget 挂到聊天区，实时状态区（spinner/teammate_tree）保持在最底部。"""
        chat = self.query_one("#chat-area", VerticalScroll)
        anchor = self._live_anchor()
        if anchor is not None:
            await chat.mount(widget, before=anchor)
        else:
    ...
    def _show_error(self, text: str) -> None:
        ...
        anchor = self._live_anchor()
        if anchor is not None:
            chat.mount(error_widget, before=anchor)
        else:
