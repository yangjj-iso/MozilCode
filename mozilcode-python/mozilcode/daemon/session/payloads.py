...
def parse_switch_provider_body(body: dict[str, Any]) -> SwitchProviderBody:
    thinking = body.get("thinking")
    if thinking is not None and not isinstance(thinking, bool):
        raise BodyFieldError("'thinking' must be a boolean")
    return SwitchProviderBody(
        provider_name=required_string_field(body, "provider_name"),
        thinking=thinking,
    )
