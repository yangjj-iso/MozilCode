...
跨 HTTP/WS 请求持有会话、任务、pending prompt 等。"""
...
class DaemonServer:
    ...
    def __init__(
        ...
        self._agent_task_runner = AgentTaskRunner(
            ...
            persist_conversation=self._persist_conversation,
    ...
    def _persist_conversation(self, sid: str, conversation: ConversationManager) -> None:
        self._records.persist_conversation(sid, conversation)
    ...
    def _emit(self, sid: str, event: dict | None) -> None:
        ...
        # Durability is an event boundary, not task completion: a daemon crash
        # must not erase an accepted prompt, a permission request, or output.
        self._persist_events(sid)
        runtime = self._agents.get(sid)
        if runtime is not None:
            self._persist_conversation(sid, runtime.conversation)
    ...
    async def set_session_provider(
        self, sid: str, provider_name: str, thinking: bool | None = None
    ) -> dict:
        """Switch the model provider used by an existing session."""
        ...
        status["provider_switched"] = result
        return status
