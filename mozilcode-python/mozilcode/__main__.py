"""CLI / 程序入口（python -m mozilcode）。

解析命令行参数、加载配置与 Hook，启动 TUI 或其它运行模式。
"""
...
def main() -> None:
    ...
    try:
        from mozilcode.account.bootstrap import ensure_runtime_config

        config = ensure_runtime_config() if args.p is None else load_config()
