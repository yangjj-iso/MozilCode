...
AppConfig / ProviderConfig 等模型，以及 load_config 合并加载。"""
...
def _with_account_providers(config: AppConfig | None) -> AppConfig | None:
    """注入账号会话合成的 gateway providers（不落盘 config.yaml）。"""
    try:
        from mozilcode.account import load_account_providers, merge_account_providers
    except Exception:
        return config

    try:
        account_providers = load_account_providers()
    except Exception:
        account_providers = []
    if not account_providers:
        return config
    if config is None:
        return AppConfig(providers=account_providers)
    return AppConfig(
        providers=merge_account_providers(account_providers, config.providers),
        ...
        memory_declared=config.memory_declared,
        enable_fork=config.enable_fork,
        enable_fork_declared=config.enable_fork_declared,
        enable_verification_agent=config.enable_verification_agent,
        enable_verification_agent_declared=config.enable_verification_agent_declared,
        worktree=config.worktree,
        worktree_declared=config.worktree_declared,
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    merged = _with_account_providers(merged)
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .mozilcode/config.yaml "
            "in project or ~/.mozilcode/config.yaml, or sign in to MozilCode Cloud"
        )
