...
定义了 AgentTool 工具，LLM 调用此工具来派生子 Agent 处理任务。

三条执行路径：
1. _execute_as_teammate: team_name 不为空时，创建 Team 队友
   （长期运行 + worktree 隔离 + 终端面板 / 进程内启动）
2. _execute_with_worktree: subagent_type 的 isolation 为 "worktree" 时，
   创建 worktree 隔离子 Agent（一次性，执行完自动清理）
