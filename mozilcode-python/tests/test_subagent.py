...
from mozilcode.agents.parser import AgentDef, AgentParseError, parse_agent_file, parse_frontmatter
...
from mozilcode.agents.fork import (
    FORK_BOILERPLATE_TAG,
    ForkError,
    build_forked_messages,
)
from mozilcode.agents.trace import TraceManager, TraceNode
from mozilcode.agents.task_manager import BackgroundTask, TaskManager
...
class TestAgentLoader:
    ...
    def test_project_overrides_builtin(self, tmp_path: Path):
        agents_dir = tmp_path / ".mozilcode" / "agents"
...
class TestToolFilter:
    ...
    def test_mcp_tools_bypass_agent_filters(self):
        reg = make_registry("ReadFile", "SomeOtherTool", "mcp_github_search")
        definition = AgentDef(
            agent_type="test",
            when_to_use="test",
            tools=["ReadFile"],
        ...
        filtered = resolve_agent_tools(reg, definition, is_background=True)
        names = {t.name for t in filtered.list_tools()}
        assert names == {"ReadFile", "mcp_github_search"}
...
class TestPermissionMode:
    def test_dont_ask_mode(self):
        from mozilcode.permissions.modes import PermissionMode, mode_decide
