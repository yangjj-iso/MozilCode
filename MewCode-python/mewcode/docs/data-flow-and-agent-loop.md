...
1. **入口层**
   - `mewcode/__main__.py`：CLI 入口，负责读取配置、初始化 hooks、进入交互 TUI 或 `-p` 非交互模式。
   - `mewcode/app.py`：Textual TUI，负责用户输入、命令分发、渲染流式输出、权限弹窗、会话保存/恢复。

2. **对话状态层**
   - `mewcode/conversation.py`：定义内部消息结构 `Message`、工具调用块 `ToolUseBlock`、工具结果块 `ToolResultBlock`，以及 `ConversationManager`。
   - 所有多轮上下文都落在 `ConversationManager.history` 里。

3. **模型适配层**
   - `mewcode/serialization.py`：把内部 `Message` 序列化成 Anthropic、OpenAI Responses、OpenAI Chat Completions 三种协议需要的请求格式。
   - `mewcode/client.py`：封装不同 provider，把各家 streaming 响应统一转成项目内部的 `StreamEvent`。

4. **Agent 编排层**
   - `mewcode/agent.py`：核心循环。负责组装上下文、调用模型、收集模型输出、执行工具、把工具结果写回历史，并决定是否继续下一轮。

5. **工具层**
   - `mewcode/tools/base.py`：工具基类、工具结果、streaming 事件类型。
   - `mewcode/tools/__init__.py`：`ToolRegistry`，负责工具注册、启停、schema 输出、延迟工具发现。
   - `mewcode/tools/*.py`：内置工具，如 `ReadFile`、`WriteFile`、`EditFile`、`Bash`、`Glob`、`Grep`、`Agent`、`ToolSearch`、`AskUserQuestion`。
   - `mewcode/mcp/*`：把 MCP server 暴露的工具包装成统一 `Tool`。

6. **上下文与持久化层**
   - `mewcode/context/manager.py`：工具结果预算、大输出持久化、自动 compact、压缩恢复附件。
   - `mewcode/memory/session.py`：会话 JSONL 持久化、恢复、compact boundary 记录。
   - `mewcode/memory/*`：长期记忆加载、召回、抽取。
...
交互模式的多轮对话主要由 `MewCodeApp` 和同一个 `ConversationManager` 实例共同维持。
...
```text
ChatInput.Submitted
  -> MewCodeApp.on_chat_input_submitted
  -> MewCodeApp._dispatch_command
```

如果输入是 `/xxx`，走命令系统；如果不是命令，走：

```text
MewCodeApp._send_message(text)
```
...
```text
.mewcode/sessions/<session_id>.jsonl
.mewcode/sessions/<session_id>.meta
```
...
1. 执行后立即处理：
   ...
   - 超过 `SINGLE_RESULT_CHAR_LIMIT` 的结果写入 `.mewcode/session/tool-results/<tool_use_id>.txt`
   - 对话里只保留预览和文件路径
...
1. `mewcode/agent.py::Agent.run`：最重要，决定一次请求如何循环。
2. `mewcode/conversation.py`：理解内部消息结构。
3. `mewcode/serialization.py`：理解内部消息怎样映射到各家模型 API。
4. `mewcode/client.py`：理解 streaming 响应怎样归一化。
5. `mewcode/tools/__init__.py` 和 `mewcode/tools/base.py`：理解工具注册和 schema。
6. `mewcode/permissions/checker.py`：理解工具调用安全边界。
7. `mewcode/context/manager.py`：理解长对话、工具大输出、compact。
8. `mewcode/memory/session.py`：理解多轮会话如何持久化和恢复。
